...
2. Separate the `MainPart` and the `MetadataPart`.
   1. `MainPart` consists of consecutive bytes which are not related to the metadata hash (all bytes are the same for both local c\_reationTxInputs\_).\
      `MetadataPart` consists of consecutive bytes related to metadata hash (some bytes differ between local c\_reationTxInputs\_).
